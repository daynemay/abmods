...
DEFINE INPUT        PARAMETER phTempTableHandle AS HANDLE    NO-UNDO.
...
------------------------------------------------------------------------------*/
    ...
    IF NOT VALID-HANDLE ( phTempTableHandle ) THEN
    DO:
        MESSAGE "An invalid temp-table handle has been passed in to" PROGRAM-NAME ( 1 ) VIEW-AS ALERT-BOX.
        APPLY "CLOSE" TO THIS-PROCEDURE.
    END.
    ...
    DO WITH FRAME {&FRAME-NAME}:
        ...
        ASSIGN
            fiFromTitle:SCREEN-VALUE    = "Fields in " + phTempTableHandle:NAME + ":"
    ...
    DO liI = 1 TO phTempTableHandle:NUM-FIELDS:

        lcFieldName = phTempTableHandle:BUFFER-FIELD ( liI ):NAME.

        IF NOT CAN-DO ( pcFieldList, lcFieldName) THEN
            NEXT.
